...
/**
 * ★選單 vs 母版 雙向稽核★（改模板或改選單後都跑一次）
 * 一次抓出所有「選單與母版對不上」的問題，不用等師傅在現場踩到：
 *   A. 選單有、母版沒格子 → 師傅點了會跳「找不到定位鍵」
 *   B. 母版有格子、選單漏掉 → 那格永遠空白，交件才發現 ★最容易被忽略★
 *   C. 母版重複定位鍵      → 同名多槽位，程式會依序填入（正常，僅告知）
 *   D. 選單內部重複        → 同一定位鍵出現在多個地方
 *
 * 執行方式：函式下拉選單選 auditMenuVsMaster → 執行 → 看執行記錄
 */
function auditMenuVsMaster() {
  const ss = SpreadsheetApp.openById(MASTER_SHEET_ID);
  const sh = ss.getSheetByName(TEMPLATE_TAB_NAME);
  if (!sh) throw new Error('找不到「' + TEMPLATE_TAB_NAME + '」分頁');

  // ① 用與 rebuildZColumnFor_ 完全相同的邏輯，算出母版真實槽位
  const lastRow = sh.getLastRow();
  ...
  Object.keys(MENU_TREE).forEach(function (r) {
    Object.keys(MENU_TREE[r]).forEach(function (eq) {
      MENU_TREE[r][eq].forEach(function (it) {
        ...
        if (!menu[k]) menu[k] = [];
        menu[k].push(r + ' / ' + eq + ' / ' + it.label);
      });
    });
  });

  const mKeys = Object.keys(master), nKeys = Object.keys(menu);
  let slotTotal = 0;
  mKeys.forEach(function (k) { slotTotal += master[k].length; });

  const out = [];
  out.push('══════ 選單 vs 母版 雙向稽核 ══════');
  out.push('母版槽位 ' + slotTotal + ' 格（定位鍵 ' + mKeys.length + ' 個）');
  out.push('選單項目 ' + menuCount + ' 項（定位鍵 ' + nKeys.length + ' 個）');

  // A：選單有、母版沒有
  const a = nKeys.filter(function (k) { return !master[k]; });
  out.push('\n【A】選單有、母版沒格子（點了會失敗）：' + (a.length ? a.length + ' 項 ✗' : '無 ✓'));
  a.forEach(function (k) { out.push('   ✗ ' + menu[k][0] + '  → ' + k); });

  // B：母版有、選單沒有（最容易被忽略）
  const b = mKeys.filter(function (k) { return !menu[k]; });
  let lost = 0;
  b.forEach(function (k) { lost += master[k].length; });
  out.push('\n【B】母版有格子、選單漏掉（該格永遠空白）：' +
           (b.length ? b.length + ' 鍵 / ' + lost + ' 格 ✗' : '無 ✓'));
  b.forEach(function (k) { out.push('   ✗ ' + k + '  → 列 ' + master[k].join(', ')); });

  // C：母版同名多槽位
  const c = mKeys.filter(function (k) { return master[k].length > 1; });
  out.push('\n【C】母版同名多槽位（程式會依序填入，正常）：' + (c.length ? c.length + ' 鍵' : '無'));
  c.forEach(function (k) { out.push('   ⚠ ' + k + ' → ' + master[k].length + ' 格'); });

  // D：選單內部重複
  const d = nKeys.filter(function (k) { return menu[k].length > 1; });
  out.push('\n【D】選單內部重複：' + (d.length ? d.length + ' 鍵 ✗' : '無 ✓'));
  d.forEach(function (k) { out.push('   ⚠ ' + k + '：' + menu[k].join(' ｜ ')); });

  // 各機房覆蓋率
  out.push('\n【各機房覆蓋率】');
  const rooms = {};
  mKeys.forEach(function (k) { rooms[k.split('__')[0]] = 1; });
  nKeys.forEach(function (k) { rooms[k.split('__')[0]] = 1; });
  Object.keys(rooms).sort().forEach(function (r) {
    let slots = 0, menus = 0, miss = 0, extra = 0;
    mKeys.forEach(function (k) {
      if (k.indexOf(r + '__') === 0) { slots += master[k].length; if (!menu[k]) miss++; }
    });
    nKeys.forEach(function (k) {
      if (k.indexOf(r + '__') === 0) { menus++; if (!master[k]) extra++; }
    });
    out.push('   ' + r + '：母版 ' + slots + ' 格 / 選單 ' + menus + ' 項 → ' +
             (miss || extra ? (miss ? '✗漏' + miss : '') + (extra ? '✗多' + extra : '') : '✓'));
  });

  const pass = (a.length === 0 && b.length === 0 && d.length === 0);
  out.push('\n══════ 結論：' + (pass ? '✅ 全部通過，選單與母版完全對應' :
           '❌ 有 ' + (a.length + b.length + d.length) + ' 項需修正') + ' ══════');
